...
export function OperationalDashboard() {
  ...
  // Enhanced KPI Data with comprehensive performance indicators
  ...
  // Enhanced System Alerts with more comprehensive monitoring
  const alerts: Alert[] = [
    {
      id: '1',
      type: 'outage',
  ...
  // Enhanced Recent Activity with more comprehensive tracking
  ...
  const getKPIsByRole = () => {
    if (userRole === 'coder') {
      return kpiData.filter(kpi => ['charges', 'claims', 'denial_rate', 'first_pass_rate', 'clean_claim_rate'].includes(kpi.id));
    }
    if (userRole === 'biller') {
      return kpiData.filter(kpi => ['payments', 'ar_days', 'tfl_warnings', 'net_collection_rate', 'aging_over_90'].includes(kpi.id));
    }
    return kpiData;
  };

  const getQueuesByRole = () => {
    if (userRole === 'coder') {
                ...
                >
                  ...
                  <div className="flex items-end justify-between">
                    <div>
                      ...
                      <div className={`flex items-center gap-1 text-xs ${kpi.trend === 'up' ? 'text-green-600' :
              ...
              {getQueuesByRole().map((queue) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full ${queue.priority === 'high' ? 'bg-red-100 text-red-700' :
                  ...
                  <div className="flex items-end justify-between">
                    <div>
                      ...
                      <div className={`flex items-center gap-1 text-xs ${queue.trend === 'up' ? 'text-red-600' :
                    ...
                    <div key={alert.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`px-2 py-1 text-xs rounded-full ${alert.severity === 'high' ? 'bg-red-100 text-red-700' :
                ...
                <div className="space-y-4 max-h-80 overflow-y-auto">
                  {recentActivity.slice(0, 8).map((activity) => (
                    <div key={activity.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                      <div className={`w-3 h-3 rounded-full flex-shrink-0 mt-1 ${activity.type === 'coding' ? 'bg-[#62d5e4]' :
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`px-2 py-1 text-xs rounded-full ${activity.type === 'coding' ? 'bg-[#62d5e4] bg-opacity-10 text-[#62d5e4]' :
